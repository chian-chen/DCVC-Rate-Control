...
template <typename scalar_t, typename vec_t>
__forceinline__ void
process_with_mask_dispatcher(torch::Tensor& y_res, torch::Tensor& y_q,
                             torch::Tensor& y_hat, torch::Tensor& s_hat,
                             const torch::Tensor& y, const torch::Tensor& scales,
                             const torch::Tensor& means, const torch::Tensor& mask,
                             const float force_zero_thres)
{
    // auto [blockDim, gridDim, stream, useVec, /*biasSafe*/ std::ignore, N, /*HW*/ std::ignore] =
    //     get_kernel_launch_info<vec_t>(y);
    auto [blockDim, gridDim, stream, useVec, biasSafeTmp, N, hwTmp] =
        get_kernel_launch_info<vec_t>(y);
    (void)biasSafeTmp;  // 避免未使用警告
    (void)hwTmp;
    ...
    if (useVec) {
        if (force_zero) {
            process_with_mask_kernel<scalar_t, vec_t, true>
                <<<gridDim, blockDim, 0, stream>>>(y_res, y_q, y_hat, s_hat,
                                                   y, scales, means, mask,
                                                   static_cast<scalar_t>(force_zero_thres), N);
        } else {
            process_with_mask_kernel<scalar_t, vec_t, false>
                <<<gridDim, blockDim, 0, stream>>>(y_res, y_q, y_hat, s_hat,
                                                   y, scales, means, mask,
                                                   static_cast<scalar_t>(force_zero_thres), N);
        }
        ...
        if (force_zero) {
            process_with_mask_kernel<scalar_t, scalar_t, true>
                <<<gridDim, blockDim, 0, stream>>>(y_res, y_q, y_hat, s_hat,
                                                   y, scales, means, mask,
                                                   static_cast<scalar_t>(force_zero_thres), N);
        } else {
            process_with_mask_kernel<scalar_t, scalar_t, false>
                <<<gridDim, blockDim, 0, stream>>>(y_res, y_q, y_hat, s_hat,
                                                   y, scales, means, mask,
                                                   static_cast<scalar_t>(force_zero_thres), N);
        }
...
template <typename scalar_t, typename vec_t>
__forceinline__ void
build_index_dec_dispatcher(torch::Tensor& out,
                           torch::optional<torch::Tensor>& cond_out,
                           const torch::Tensor& scales,
                           const scalar_t scale_min,
                           const scalar_t scale_max,
                           const scalar_t log_scale_min,
                           const scalar_t log_step_recip,
                           const scalar_t skip_thres)
{
    auto [blockDim, gridDim, stream, useVec, N] =
        get_kernel_launch_info_flatten<vec_t>(scales);

    const bool with_cond = static_cast<float>(skip_thres) > 0.f;

    if (with_cond) {
        if (useVec) {
            build_index_dec_kernel<scalar_t, vec_t, uchar4, bool4, true>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, cond_out.value(), scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
                    skip_thres, N);
        } else {
            build_index_dec_kernel<scalar_t, scalar_t, uint8_t, bool, true>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, cond_out.value(), scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
        ...
        if (useVec) {
            build_index_dec_kernel<scalar_t, vec_t, uchar4, bool4, false>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, nullptr, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
            ...
            build_index_dec_kernel<scalar_t, scalar_t, uint8_t, bool, false>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, nullptr, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
...
template <typename scalar_t, typename vec_t>
__forceinline__ void
build_index_enc_dispatcher(torch::Tensor& out,
                           torch::optional<torch::Tensor>& cond_out,
                           ...
                           const scalar_t skip_thres)
{
    ...
    if (with_cond) {
        if (useVec) {
            build_index_enc_kernel<scalar_t, vec_t, short4, bool4, true>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, cond_out.value(), symbols, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
                    skip_thres, N);
        } else {
            build_index_enc_kernel<scalar_t, scalar_t, int16_t, bool, true>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, cond_out.value(), symbols, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
        ...
        if (useVec) {
            build_index_enc_kernel<scalar_t, vec_t, short4, bool4, false>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, nullptr, symbols, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
                    skip_thres, N);
        } else {
            build_index_enc_kernel<scalar_t, scalar_t, int16_t, bool, false>
                <<<gridDim, blockDim, 0, stream>>>(
                    out, nullptr, symbols, scales,
                    scale_min, scale_max,
                    log_scale_min, log_step_recip,
