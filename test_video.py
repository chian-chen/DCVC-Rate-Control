...
def parse_args():
    ...
    parser.add_argument('--verbose_json', type=str2bool, default=True)
...
def run_one_point_with_stream(p_frame_net, i_frame_net, args):
    ...
    # index_map = [0, 0, 0, 0, 0, 0, 0, 0]  # use the same qp for all frames
    ...
    with torch.no_grad():
        ...
        for frame_idx in range(frame_num):
            ...
            # print(stream_bytes * 8 + sps_bytes * 8)
    ...
    print("bits mean:" , sum(bits)/len(bits))
    src_reader.close()
...
def main():
    ...
    #                                                              initargs=(args, gpu_num))
    # objs = []
    init_func(args, gpu_num)        # load+move i_frame_net, p_frame_net onto cuda:0
    objs = []
